...
export default {
  ...
  head () {
    return {
      title: this.post.title + ' | Travis Supply',
      meta: [
        ...
        { hid: 'og:image', name: 'og:image', content: this.post.photo_cover }
      ]
    }
  },
  data () {
  ...
  async asyncData ({ params, app }) {
    const { data } = await request.getPost(params.id)
    app.post2 = data
    return { post: data, post2: app.post2 }
  },
  created () {
    console.log('app', this.post2)
  ...
  methods: {
    ...
    initVideoPlayer () {
      if (process.client && document.getElementById('video-scroll')) {
        let player
